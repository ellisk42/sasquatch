...
def full_printer(m):
    ...
    for n in range(len(observations)):
        program += "\nObservation %i:\n\t" % n
        for d in inputs[n][1]['distances']:
            program = program + ("%s = %f; " % (str(d), extract_real(m,d)))
        program += "\n\t"
        ...
        program += "\n\t"
        for sh in inputs[n][1]['shapes']:
            program = program + ("%s = %f; " % (str(sh), extract_real(m,sh)))
        program = program + "\n"
    return program
...
parallelCompression(full_printer,mdl,[[boolean()]])
